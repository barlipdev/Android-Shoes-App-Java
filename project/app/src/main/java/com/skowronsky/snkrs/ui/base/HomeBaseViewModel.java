...
public class HomeBaseViewModel extends AndroidViewModel {
    ...
    public void deleteBaseShoes(Shoes shoe, double size){
        ...
        for (int i = 0; i < allBaseShoes.getValue().size(); i++){
                    for (int j = 0; j < allBaseShoes.getValue().get(i).shoes.size(); j++) {
                        if (model_name.equals(allBaseShoes.getValue().get(i).shoes.get(j).modelName) && size == allBaseShoes.getValue().get(i).base.size){
                                repository.delete(allBaseShoes.getValue().get(i).base);
                                Log.i("ROOM114", String.valueOf("Size: "+allBaseShoes.getValue().get(i).shoes.size()));
                                Log.i("ROOM114", String.valueOf("Shoe: "+shoe.getModelName()));
                                Log.i("ROOM114", String.valueOf("Deleted: "+allBaseShoes.getValue().get(i).shoes.get(j).modelName));
                    }
                }
        }
    }
